...
import time
from math import radians, cos, asin, sqrt, log, tan, pi
import random
...
# =========================
# Satellite (Google → Esri fallback) & Features
# =========================
...
# ------------------------------
# Brand / Chain calibration
# ------------------------------

KNOWN_CHAINS = {
    "taco bell", "mcdonald", "burger king", "wendy", "chipotle", "kfc", "subway",
    "chick-fil-a", "domino", "papa john", "little caesars", "dunkin", "starbucks",
    "panera", "five guys", "shake shack", "popeyes", "arbys", "sonic", "culver",
}

def is_chain(store_name: str, yelp_details: Optional[dict]) -> bool:
    name = (store_name or "").lower()
    if any(k in name for k in KNOWN_CHAINS):
        return True
    if yelp_details:
        yn = str(yelp_details.get("name", "")).lower()
        if any(k in yn for k in KNOWN_CHAINS):
            return True
        url = str(yelp_details.get("url", "")).lower()
        if any(k.replace(" ", "-") in url for k in KNOWN_CHAINS):
            return True
    return False
...
def _soft_clip(x: float, lo: float, hi: float) -> float:
    ...
    s = span * 0.05
    if x < lo:
        return lo + (np.tanh((x - lo) / s) + 1) * (s / 2)
    if x > hi:
        return hi - (np.tanh((x - hi) / s) + 1) * (s / 2)
    return x


def hybrid_prediction(model, model_feats, aux_feats, *, store_name: str = "", store_type: str = "Any", yelp_details: Optional[dict] = None):
    # 1) Model
    try:
        model_pred = float(model.predict([model_feats])[0])
    ...
    # 3) Brand calibration
    chain_flag = is_chain(store_name, yelp_details)
    size_mult = business_size_factor(store_type, chain_flag, int(reviews or 0))
...
def generate_recommendations(store, store_type, foot, soc, sales, fred, *, n=8, seed=None):
    ...
    recs = set()
    recs.update(rng.sample(pool, k=min(3, len(pool))))
    ...
    if macros:
        recs.update(rng.sample(macros, k=min(1, len(macros))))
    icons = []
    for item in recs:
        if "review" in item.lower():
            ...
            icons.append("🧺 " + item)
        elif any(k in item.lower() for k in ["loyal", "stamp", "vip"]):
            icons.append("🎟️ " + item)
        elif any(k in item.lower() for k in ["reels", "influencer", "ugc"]):
            icons.append("📣 " + item)
        else:
            icons.append("💡 " + item)
    return icons[:n]
...
if store:
    ...
    if isinstance(zc, tuple):
        ...
        lat, lon, lab = ROBBINSVILLE_CENTER[0], ROBBINSVILLE_CENTER[1], "Robbinsville default"
...
if coords:
    ...
    if yelp_details:
        ...
        with left:
            ...
            if yelp_details.get("price"):
                title += f" · {yelp_details['price']}"
            if yelp_details.get("is_closed") is True:
                title += " · _Closed_"
            st.markdown(title)
            st.write(
                f"**Rating**: {yelp_details.get('rating','—')} {_stars(yelp_details.get('rating'))}  ·  "
                f"**Reviews**: {yelp_details.get('review_count','—')}"
            )
            cats = (
                ", ".join([c.get("title", "") for c in yelp_details.get("categories", [])])
                if isinstance(yelp_details.get("categories"), list)
                else yelp_details.get("categories", "")
            )
            ...
            if tx:
                st.write("**Transactions**: " + ", ".join(tx))
            st.write(f"**Phone**: {yelp_details.get('display_phone') or yelp_details.get('phone') or '—'}")
            if yelp_details.get("url"):
                st.write(f"[Open on Yelp]({yelp_details['url']})")
    ...
    if st.button("📊 Predict & Analyze"):
        try:
            ...
            aux_dict = {
                "foot": float(aux_feats[3]),
                "yelp_sent": float(aux_feats[2]),
                "rating": float(aux_feats[0]),
                "reviews": float(aux_feats[1]),
                "rrsfs_z": float(aux_feats[4]),
                "umcsent_z": float(aux_feats[5]),
                "unrate_z": float(aux_feats[6]),
            }
            save_prediction(store, coords, pred, aux_dict)
            plot_insights(store)
